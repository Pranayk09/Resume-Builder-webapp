...
const TemplateSelector = ({selectedTemplate, onChange}) => {
  ...
  return (
    <div className='relative '>
        ...
        {
            isOpen && (
                <div className='absolute top-full mt-2 w-64 p-3 space-y-3 z-50
                bg-white rounded-md border border-gray-200 shadow-sm
                max-h-64 overflow-y-auto'>
                    {templates.map((template)=>(
                        <div onClick={()=>{onChange(template.id); setIsOpen(false)}} key={template.id} className={`relative p-3 border rounded-md cursor-pointer transition-all ${selectedTemplate === template.id ? 'border-blue-400 bg-blue-100': 'border-gray-300 hover:border-gray-400 hover:bg-gray-100' } `}>
